...
from RANK import AMat, BMat, CMat, EMat
...
# q = q.conj().T
# note q' @ Lambda @ z' - G = 0 (see eq 4.49 in DeJong and Dave)
...
tmp = np.hstack((O11, O12-Phi@O22  ))
aux = tmp @ z.conj().T

maux, mO11, mO12, mPhi,mO22,mz, mtmp =[ np.array(xx) for xx in eng.checkeu_debug(MConv(g0),MConv(g1),MConv(psi),MConv(pi),1+1e-10,nargout = 7 )]
