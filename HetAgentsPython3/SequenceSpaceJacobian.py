...
def BackwardIterationZ(Kguess,Zpath):
...
def BackwardIterationK(Kguess,Zpath):
    calD = np.zeros((N*Asize,TimePeriods))
    calY = np.zeros(TimePeriods)
    ...
    for u in range(TimePeriods):
        ...
        calD[ :,  u] = (MakeTransMat(G)@D_SS - D_SS)/eps
        calY[ u ] = (np.dot(np.tile(A,(1,N)),D_SS) - K)/eps  # the individual outcomes do not depend on the policy rules in this case
...
calD_K,calY_K = BackwardIterationK(Kpert,Z)
calD_Z,calY_Z = BackwardIterationZ(K,Zpert)
...
dZ = 0.01*(np.arange(TimePeriods)[:, np.newaxis] == np.array([5,10, 15, 20, 25]))
G_K = -numpy.linalg.inv(Jac_K) @ Jac_Z
dK = G_K @ dZ
plt.figure()
plt.plot(dK[:50])
plt.show()
